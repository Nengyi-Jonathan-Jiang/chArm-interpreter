...
export const DummyScrollSyncer: IScrollSyncer = {
    registerPane:   () => void 0,
    unregisterPane: () => void 0,
};

export const ScrollSyncContext = createContext<IScrollSyncer>(
    DummyScrollSyncer);
...
export class ScrollPane<ElementType extends ScrollableElement = any> {
    ...
    set scrollProportionX (proportion: number) {
        this.element.scrollLeft = clamp(
            proportion, 0, 1) * this.scrollableAmountX;
    }
    ...
    set scrollProportionY (proportion: number) {
        this.element.scrollTop = clamp(
            proportion, 0, 1) * this.scrollableAmountY;
    }
...
abstract class AbstractScrollSyncer implements IScrollSyncer {
    ...
    protected abstract syncY (
        sourcePane: ScrollPane, destPane: ScrollPane): void;

    protected abstract syncX (
        sourcePane: ScrollPane, destPane: ScrollPane): void;
...
export class ProportionalScrollSyncer extends AbstractScrollSyncer {
    ...
    protected syncY (
        sourcePane: { readonly scrollProportionY: number },
        destPane: { scrollProportionY: number },
    ) {
        destPane.scrollProportionY = sourcePane.scrollProportionY;
    }

    protected syncX (
        sourcePane: { readonly scrollProportionX: number },
        destPane: { scrollProportionX: number },
    ) {
